...
if __name__ == "__main__":
    ...
    config['weights'] = args.weight_ocr
    ...
    for image_file in image_file_list:
        ...
        img_name = img_name_pure.replace(".jpg","").replace("jpeg","")
        img_path = os.path.join(draw_img_save,
                                "det_res_{}".format(img_name_pure))
        cv2.imwrite(img_path, src_im)
        logger.info("The visualized image saved in {}".format(img_path))
        boxes = [line for line in dt_boxes]
        txts = [line[1][0] for line in dt_boxes]
        scores = [line[1][1] for line in dt_boxes]
        ocr_result = ""
        for box in boxes: 
            box[0][1] = box[0][1] * 0.75
            box[2][1]= box[2][1] *1.3
            box[0][0] = box[0][0] *0.7
            box[2][0]= box[2][0] *1.3
        for box in boxes:    
            crop_img = align(img,box)
            convert_img = cv2.cvtColor(crop_img, cv2.COLOR_BGR2RGB)
            im_pil = Image.fromarray(convert_img)
            s = ocr.predict(im_pil)
            ocr_result = s + ' ' + ocr_result
            cv2.imwrite("./{0}/{1}".format(args.inference_results,img_name_pure),crop_img)
        ocr_result = remove_accent(ocr_result)
        with open(os.path.join(draw_img_save, f"ocr_{img_name}.txt"), 'w') as f:
            tmp = ""
            print('ocr_result', ocr_result)
            if check_is_reaction(processed_lst, ocr_result):
                tmp = "Phản động"
            else:
                tmp = "Bình thường"
            f.writelines(tmp+"\n")
